...
begin
  if Trim(AJSONString).StartsWith('{') then
//    LoadFromJSON({$IF DEFINED(FPC)}GetJSON(AJSONString){$ELSE}TJSONObject.ParseJSONValue(TEncoding.ANSI.GetBytes(AJSONString), 0){$ENDIF} as TJSONObject)
...
//    LoadFromJSON({$IF DEFINED(FPC)}GetJSON(AJSONString){$ELSE}TJSONObject.ParseJSONValue(TEncoding.ANSI.GetBytes(AJSONString), 0){$ENDIF} as TJSONArray);
end;
